...
class TaoEval:
    ...
    def evaluate(self, show_progress=False):
        ...
        self.eval_vids = {(v, c, a, t):
                          self.evaluate_vid(vid_id, cat_id, area_rng, time_rng, use_occlusion=(a == len(self.params.area_rng) - 1))
    ...
    def evaluate_vid(self, vid_id, cat_id, area_rng, time_rng, use_occlusion=False):
        """Perform evaluation for single category and video.
        
        Args:
            use_occlusion: if True, we use visibility of each track for filtering.
        """
        gt, dt = self._get_gt_dt(vid_id, cat_id)
        if len(gt) == 0 and len(dt) == 0:
            return None
        ...
        if not use_occlusion:
            ...
            for g in gt:
                duration = len(g['annotations'])
                num_HP_frames =  sum(x['visibility'] < 0.8 for x in g['annotations'])
                if (g["ignore"]
                        or (g["area"] < area_rng[0] or g["area"] > area_rng[1])
                        or (duration < time_rng[0] or duration > time_rng[1])
                        or (num_HP_frames <= 5)):
                    # when number of Highly/Partially occluded frames <= 5, we ignore this track
                    g["_ignore"] = 1
                else:
                    g["_ignore"] = 0
    ...
    def summarize(self):
        ...
        self.results["AP-HP"] = self._summarize('ap', area_rng="highly-and-partially-occluded")
        self.results["AP50-HP"] = self._summarize('ap', area_rng="highly-and-partially-occluded", iou_thr=0.50)
        self.results["AP75-HP"] = self._summarize('ap', area_rng="highly-and-partially-occluded", iou_thr=0.75)
...
class Params:
    def __init__(self, iou_type, iou_3d_type='3d_iou'):
        ...
        self.area_rng_lbl = ["all", "small", "medium", "large", "highly-and-partially-occluded"]
