...
def visualize(coco, video: str, labeled_frames, args, tao_amodal_prediction):
    ...
    with video_writer(str(output_video), (width, height)) as writer:
        ...
        for i, frame in enumerate(frames):
            ...
            full_image = np.ones((int(height*1.5), int(width*1.5), 3), dtype=np.uint8) * 255
            startx = int(width/4)
            endx = startx + width
            starty = int(height/5)
            endy = starty + height
            ...
            * Plot the amodal annotation
            ...
            if is_annotated or is_interpolated:
                ...
                if args.transparent:
                    visualized = vis_utils.transparent_except_bbox(visualized, predictions, oy=starty,
...
def main():
    ...
    * Select the videos we need
    ...
    for video, labeled_frames in videos.items():
        output_video = args.output_dir / (video + '.mp4')
        tasks.append((coco, video, labeled_frames, args, tao_amodal_prediction))
