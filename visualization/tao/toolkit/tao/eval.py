...
class TaoEval:
    ...
    def evaluate(self):
        ...
        self.ious = {
            (vid_id, cat_id): self.compute_iou(vid_id, cat_id)
            for vid_id in tqdm(self.params.vid_ids, desc='Computing IoUs')
        ...
        if len(s[s > -1]) == 0:
            ...
            if not categories:
                ...
                for i in range(s.shape[2]):
                    sliced = s[:, :, i, :]
                    if len(sliced[sliced > -1]) == 0:
                        mean = -1
                    else:
                        mean = np.mean(sliced[sliced > -1])
                    mean_s.append(mean)
    ...
    def summarize(self):
        ...
        self.results["APcat"] = self._summarize('ap', categories=True)
    ...
    def print_results(self):
        ...
        for key, value in self.results.items():
            ...
            if 'cat' not in key:
                ...
                self.logger.info(
                    template2.format(title, _type, iou, area_rng, time_rng,
                                    max_dets, cat_group_name, str(value)))
